import type { Client } from '@/features/clients/types'

export const seedClients: Client[] = [
  {
    id: '10000000-0000-4000-8000-000000000001',
    fullName: 'Ana Paula Mendes',
    cpf: '123.456.789-01',
    birthDate: '1990-03-15',
    email: 'ana.mendes@email.com',
    phone: '(11) 98765-4321',
    profession: 'Arquiteta',
    zipCode: '01310-100',
    city: 'São Paulo',
    state: 'SP',
    address: 'Av. Paulista, 1000',
    complement: 'Apto 42',
    interest: 'Compra',
    budgetRange: 'R$ 100–200 mil',
    howFoundUs: 'Instagram',
    status: 'active',
    visits: 4,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000002',
    fullName: 'Carlos Eduardo Silva',
    cpf: '234.567.890-12',
    birthDate: '1985-07-22',
    email: 'carlos.silva@email.com',
    phone: '(21) 99876-5432',
    profession: 'Engenheiro',
    zipCode: '22041-080',
    city: 'Rio de Janeiro',
    state: 'RJ',
    address: 'Rua Visconde de Pirajá, 500',
    complement: 'Cobertura 801',
    interest: 'Financiamento',
    budgetRange: 'R$ 50–100 mil',
    howFoundUs: 'Google',
    status: 'negotiating',
    visits: 7,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000003',
    fullName: 'Fernanda Costa Oliveira',
    cpf: '345.678.901-23',
    birthDate: '1995-11-08',
    email: 'fernanda.costa@email.com',
    phone: '(31) 97654-3210',
    profession: 'Médica',
    zipCode: '30130-000',
    city: 'Belo Horizonte',
    state: 'MG',
    address: 'Av. Afonso Pena, 1500',
    complement: 'Sala 3',
    interest: 'Troca',
    budgetRange: 'Acima de R$ 200 mil',
    howFoundUs: 'Indicação',
    status: 'active',
    visits: 2,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000004',
    fullName: 'João Pedro Almeida',
    cpf: '456.789.012-34',
    birthDate: '1988-01-30',
    email: 'joao.almeida@email.com',
    phone: '(41) 96543-2109',
    profession: 'Empresário',
    zipCode: '80010-000',
    city: 'Curitiba',
    state: 'PR',
    address: 'Rua XV de Novembro, 200',
    complement: 'Sala 1204',
    interest: 'Venda',
    budgetRange: 'Até R$ 50 mil',
    howFoundUs: 'Outdoor',
    status: 'inactive',
    visits: 1,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000005',
    fullName: 'Mariana Souza Lima',
    cpf: '567.890.123-45',
    birthDate: '1992-05-12',
    email: 'mariana.lima@email.com',
    phone: '(51) 95432-1098',
    profession: 'Designer',
    zipCode: '90010-150',
    city: 'Porto Alegre',
    state: 'RS',
    address: 'Rua dos Andradas, 800',
    complement: 'Bloco B',
    interest: 'Compra',
    budgetRange: 'R$ 100–200 mil',
    howFoundUs: 'Google',
    status: 'active',
    visits: 5,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000006',
    fullName: 'Roberto Nascimento',
    cpf: '678.901.234-56',
    birthDate: '1978-09-03',
    email: 'roberto.nasc@email.com',
    phone: '(85) 94321-0987',
    profession: 'Advogado',
    zipCode: '60160-230',
    city: 'Fortaleza',
    state: 'CE',
    address: 'Av. Beira Mar, 3200',
    complement: 'Apto 1502',
    interest: 'Financiamento',
    budgetRange: 'R$ 50–100 mil',
    howFoundUs: 'Indicação',
    status: 'negotiating',
    visits: 3,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000007',
    fullName: 'Juliana Ferreira Santos',
    cpf: '789.012.345-67',
    birthDate: '1993-06-18',
    email: 'juliana.ferreira@email.com',
    phone: '(62) 99123-4567',
    profession: 'Contadora',
    zipCode: '74003-010',
    city: 'Goiânia',
    state: 'GO',
    address: 'Av. Goiás, 450',
    complement: 'Apto 78',
    interest: 'Compra',
    budgetRange: 'R$ 50–100 mil',
    howFoundUs: 'Instagram',
    status: 'active',
    visits: 6,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000008',
    fullName: 'Lucas Henrique Barbosa',
    cpf: '890.123.456-78',
    birthDate: '1987-12-05',
    email: 'lucas.barbosa@email.com',
    phone: '(71) 98234-5678',
    profession: 'Analista de Sistemas',
    zipCode: '40020-000',
    city: 'Salvador',
    state: 'BA',
    address: 'Av. Sete de Setembro, 1200',
    complement: 'Ed. Atlântico, sala 5',
    interest: 'Troca',
    budgetRange: 'R$ 100–200 mil',
    howFoundUs: 'Google',
    status: 'negotiating',
    visits: 8,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000009',
    fullName: 'Patricia Gomes Ribeiro',
    cpf: '901.234.567-89',
    birthDate: '1991-04-27',
    email: 'patricia.ribeiro@email.com',
    phone: '(27) 97345-6789',
    profession: 'Professora',
    zipCode: '29010-080',
    city: 'Vitória',
    state: 'ES',
    address: 'Av. Jerônimo Monteiro, 350',
    complement: 'Casa 2',
    interest: 'Financiamento',
    budgetRange: 'Até R$ 50 mil',
    howFoundUs: 'Outro',
    status: 'inactive',
    visits: 2,
    photoUrl: '',
  },
  {
    id: '10000000-0000-4000-8000-000000000010',
    fullName: 'Thiago Moreira Campos',
    cpf: '012.345.678-90',
    birthDate: '1984-08-14',
    email: 'thiago.campos@email.com',
    phone: '(48) 96456-7890',
    profession: 'Gerente Comercial',
    zipCode: '88010-400',
    city: 'Florianópolis',
    state: 'SC',
    address: 'Rua Felipe Schmidt, 680',
    complement: 'Apto 301',
    interest: 'Venda',
    budgetRange: 'Acima de R$ 200 mil',
    howFoundUs: 'Indicação',
    status: 'active',
    visits: 9,
    photoUrl: '',
  },
]
